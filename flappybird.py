...
# Creating a class to represent the bird
...
bird_image = pygame.transform.scale(bird_image, (bird_width, bird_height)) # transform or resize the image 
...
pipes = []
velocity_x = -2 # move pipe to the left speed (simulates bird moving right)
...
def move():
    ...
    for pipe in pipes:
        ...
        if not pipe.passed and bird.x > pipe.x + pipe.width:
            score += 0.5 # 0.5 because there are 2 pipes 0.5 * 2 = 1, 1 per set of pipe
            pipe.passed = True
    ...
    while len(pipes) > 0 and pipes[0].x < -pipe_width:
        pipes.pop(0) # remove first element from the list
...
pygame.time.set_timer(create_pipe_timer, 1500) # marks every 1.5 seconds
